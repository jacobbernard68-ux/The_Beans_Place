...
export default function NavBar() {
  const [menuOpen, setMenuOpen] = useState(false);
  //container =menuOpen, funtion to update the container = setMenuOpen, current value = false
  const [scrolled, setScrolled] = useState(false);
  ...
  return (
    ...
    >
      <div className="mx=auto flex full w-full max-w-7x1 items-center justify-between px-4 py-3 md:px-8">
        {/* Brand */}
        ...
        >
          <span
            className={
              'block h-0.5 w-6 bg-black trasition-all duration-300 ${menuOpen ? "translate-y-2 rotate-45" : ""}'
            }
          />

          <span
            className={
              'block h-0.5 w-6 bg-black transition-all duration-300 ${menuOpen ? "opacity-0" : "opacity-100}'
            }
          />
          <span
            className={
              'block h-0.5 w-6 bg-black transition-all duration-300 ${menuOpen ? "-translate-y-2 -rotate-45" : ""}'
            }
          />
        </button>
      </div>
